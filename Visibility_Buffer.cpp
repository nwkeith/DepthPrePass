...
/************************************************************************/
// Simple geometry forward rendering pipeline
/************************************************************************/
Shader*        pShaderVisibilityBufferPass[gNumGeomSets] = {};
Pipeline*      pPipelineVisibilityBufferPass[gNumGeomSets] = {};
/************************************************************************/
// Simple geometry (no depth pre pass) pipeline
/************************************************************************/
Pipeline*      pPipelineSimpleGeometryDepthWrite = {};
/************************************************************************/
// Depth pre pass pipeline
/************************************************************************/
Shader*        pShaderDepthPrePass = {};
Pipeline*      pPipelineDepthPrePass = {};
/************************************************************************/
// VB shade pipeline
/************************************************************************/
...
/************************************************************************/
// Overdraw Visibility pipeline
/************************************************************************/
Shader* pShaderOverdrawVis = { nullptr };
...
public:
    ...
    bool Load(ReloadDesc* pReloadDesc)
    {
        ...
        if (pReloadDesc->mType & (RELOAD_TYPE_RESIZE | RELOAD_TYPE_RENDERTARGET))
        {
            ...
            checkbox.pData = &gAppSettings.mUseDepthPrePass;
            luaRegisterWidget(uiAddComponentWidget(pGuiWindow, "Depth Pre Pass", &checkbox, WIDGET_TYPE_CHECKBOX));
    ...
    void addShaders()
    {
        ...
        ShaderLoadDesc depthPrePass = {};
        ShaderLoadDesc overdrawVis = {};
        ...
        // Depth pre pass dummy shader
        depthPrePass.mVert.pFileName = "OverdrawVis.vert";
        depthPrePass.mFrag.pFileName = "Depth.frag";

        // Simple overdraw visualization shader
        overdrawVis.mVert.pFileName = "OverdrawVis.vert";
        overdrawVis.mFrag.pFileName = "OverdrawVis.frag";
        ...
        addShader(pRenderer, &depthPrePass, &pShaderDepthPrePass);
        addShader(pRenderer, &overdrawVis, &pShaderOverdrawVis);
        for (uint32_t i = 0; i < MSAA_LEVELS_COUNT; ++i)
        {
    ...
    void removeShaders()
    {
        ...
        removeShader(pRenderer, pShaderDepthPrePass);
        removeShader(pRenderer, pShaderOverdrawVis);
    }
    ...
    void addPipelines()
    {
        ...
        DepthStateDesc depthStateReadOnlyDesc = {};
        depthStateReadOnlyDesc.mDepthTest = true;
        depthStateReadOnlyDesc.mDepthWrite = false;
        depthStateReadOnlyDesc.mDepthFunc = CMP_GEQUAL;
        ...
        RasterizerStateDesc rasterizerStateCullBackDesc = { CULL_MODE_BACK, 0, 0, FILL_MODE_SOLID, FRONT_FACE_CW };
        RasterizerStateDesc rasterizerStateCullBackMsDesc = { CULL_MODE_BACK, 0, 0, FILL_MODE_SOLID, FRONT_FACE_CW };
        rasterizerStateCullNoneMsDesc.mMultiSample = true;

        VertexLayout simpleVertexLayout = {};
        simpleVertexLayout.mAttribCount = 3;
        simpleVertexLayout.mBindingCount = 3;
        simpleVertexLayout.mAttribs[0].mSemantic = SEMANTIC_POSITION;
        simpleVertexLayout.mAttribs[0].mFormat = TinyImageFormat_R32G32B32_SFLOAT;
        simpleVertexLayout.mAttribs[0].mBinding = 0;
        simpleVertexLayout.mAttribs[0].mLocation = 0;
        simpleVertexLayout.mAttribs[1].mSemantic = SEMANTIC_TEXCOORD0;
        simpleVertexLayout.mAttribs[1].mFormat = TinyImageFormat_R16G16_SFLOAT;
        simpleVertexLayout.mAttribs[1].mBinding = 1;
        simpleVertexLayout.mAttribs[1].mLocation = 1;
        simpleVertexLayout.mAttribs[2].mSemantic = SEMANTIC_NORMAL;
        simpleVertexLayout.mAttribs[2].mFormat = TinyImageFormat_R16G16_UNORM;
        simpleVertexLayout.mAttribs[2].mBinding = 2;
        simpleVertexLayout.mAttribs[2].mLocation = 2;
        simpleVertexLayout.mBindings[0].mRate = VERTEX_BINDING_RATE_VERTEX;
        simpleVertexLayout.mBindings[0].mStride = 12;
        simpleVertexLayout.mBindings[1].mRate = VERTEX_BINDING_RATE_VERTEX;
        simpleVertexLayout.mBindings[1].mStride = 4;
        simpleVertexLayout.mBindings[2].mRate = VERTEX_BINDING_RATE_VERTEX;
        simpleVertexLayout.mBindings[2].mStride = 4;

        /************************************************************************/
        // Setup the resources needed for the Depth Pre Pass Pipeline
        /************************************************************************/
        // Create pipeline
        GraphicsPipelineDesc& depthPrePassPipelineSettings = pipelineDesc.mGraphicsDesc;
        depthPrePassPipelineSettings = { 0 };
        depthPrePassPipelineSettings.mPrimitiveTopo = PRIMITIVE_TOPO_TRI_LIST;
        depthPrePassPipelineSettings.mRenderTargetCount = 1;
        depthPrePassPipelineSettings.pDepthState = &depthStateDesc;
        depthPrePassPipelineSettings.mDepthStencilFormat = isMSAAEnabled ? pMSAAEdgesStencilBuffer->mFormat : pDepthBuffer->mFormat;
        depthPrePassPipelineSettings.pRasterizerState = &rasterizerStateCullBackMsDesc;
        depthPrePassPipelineSettings.pVertexLayout = &simpleVertexLayout;

        // For depth pre pass, don't write color at all
        BlendStateDesc depthBlendStateDesc = {};
        depthBlendStateDesc.mColorWriteMasks[0] = COLOR_MASK_NONE;
        depthBlendStateDesc.mRenderTargetMask = BLEND_STATE_TARGET_0;

        depthPrePassPipelineSettings.pBlendState = &depthBlendStateDesc;
        depthPrePassPipelineSettings.pShaderProgram = pShaderDepthPrePass;
        depthPrePassPipelineSettings.mSampleCount = gAppSettings.mMsaaLevel;
        if (isMSAAEnabled)
        {
            depthPrePassPipelineSettings.pColorFormats = &pRenderTargetMSAA->mFormat;
            depthPrePassPipelineSettings.mSampleQuality = pRenderTargetMSAA->mSampleQuality;
        }
        else
        {
            depthPrePassPipelineSettings.pColorFormats = &pIntermediateRenderTarget->mFormat;
            depthPrePassPipelineSettings.mSampleQuality = pSwapChain->ppRenderTargets[0]->mSampleQuality;
        }
        pipelineDesc.pName = "Depth Pre Pass";
        addPipeline(pRenderer, &pipelineDesc, &pPipelineDepthPrePass);

        /************************************************************************/
        // Setup the resources needed for the Visibility Buffer Shade Pipeline
        /************************************************************************/
        // Create pipeline
        // Vertex layout is NOT NULL because of the simpler vertex shader used here
        GraphicsPipelineDesc& simpleRenderPipelineSettings = pipelineDesc.mGraphicsDesc;
        simpleRenderPipelineSettings = { 0 };
        simpleRenderPipelineSettings.mPrimitiveTopo = PRIMITIVE_TOPO_TRI_LIST;
        simpleRenderPipelineSettings.mRenderTargetCount = 1;
        simpleRenderPipelineSettings.pDepthState = &depthStateReadOnlyDesc;
        simpleRenderPipelineSettings.mDepthStencilFormat = isMSAAEnabled ? pMSAAEdgesStencilBuffer->mFormat : pDepthBuffer->mFormat;
        simpleRenderPipelineSettings.pRasterizerState = isMSAAEnabled ? &rasterizerStateCullBackMsDesc : &rasterizerStateCullBackDesc;
        simpleRenderPipelineSettings.pVertexLayout = &simpleVertexLayout;

        // Shader variants excluding godray
        const uint32_t kNumShaderVariantsExGodray = 2 * MSAA_LEVELS_COUNT;

        BlendStateDesc overdrawBlendStateDesc = {};
        overdrawBlendStateDesc.mSrcFactors[0] = BC_ONE;
        overdrawBlendStateDesc.mDstFactors[0] = BC_ONE;
        overdrawBlendStateDesc.mBlendModes[0] = BM_ADD;
        overdrawBlendStateDesc.mSrcAlphaFactors[0] = BC_ONE;
        overdrawBlendStateDesc.mDstAlphaFactors[0] = BC_ZERO;
        overdrawBlendStateDesc.mBlendAlphaModes[0] = BM_ADD;
        overdrawBlendStateDesc.mColorWriteMasks[0] = COLOR_MASK_RED | COLOR_MASK_GREEN | COLOR_MASK_BLUE;
        overdrawBlendStateDesc.mRenderTargetMask = BLEND_STATE_TARGET_0;
        overdrawBlendStateDesc.mIndependentBlend = false;
        simpleRenderPipelineSettings.pBlendState = &overdrawBlendStateDesc;
        simpleRenderPipelineSettings.pShaderProgram = pShaderOverdrawVis;

        for (uint32_t i = 0; i < 2; ++i)
        {
            simpleRenderPipelineSettings.mSampleCount = gAppSettings.mMsaaLevel;
            if (isMSAAEnabled)
            {
                simpleRenderPipelineSettings.pColorFormats = &pRenderTargetMSAA->mFormat;
                simpleRenderPipelineSettings.mSampleQuality = pRenderTargetMSAA->mSampleQuality;
            }
            else
            {
                // vbShadePipelineSettings.pColorFormats = &pSwapChain->ppRenderTargets[0]->mFormat;
                simpleRenderPipelineSettings.pColorFormats = &pIntermediateRenderTarget->mFormat;
                simpleRenderPipelineSettings.mSampleQuality = pSwapChain->ppRenderTargets[0]->mSampleQuality;
            }
            ...
            pipelineDesc.mExtensionCount = sizeof(edescs) / sizeof(edescs[0]);
#endif
            pipelineDesc.pName = "Simple Geometry Pass";
        ...
        pipelineDesc.pName = "Simple Geometry Pass with Depth Write";
        addPipeline(pRenderer, &pipelineDesc, &pPipelineSimpleGeometryDepthWrite);
    ...
    void removePipelines()
    {
        ...
        removePipeline(pRenderer, pPipelineDepthPrePass);
        removePipeline(pRenderer, pPipelineSimpleGeometryDepthWrite);
    ...
    // Render the scene without drawing to the screen to fill depth buffer.
    // this depth buffer will be used when shading the scene to help reduce overdraw.
    void depthPrePass(Cmd* cmd, ProfileToken pGpuProfiler)
    {
        const bool    isDrawingToMSRT = gAppSettings.mMsaaLevel > SAMPLE_COUNT_1;
        RenderTarget* pDestinationRenderTarget = isDrawingToMSRT ? pRenderTargetMSAA : pScreenRenderTarget;
        RenderTarget* pDepthRenderTarget = isDrawingToMSRT ? pMSAAEdgesStencilBuffer : pDepthBuffer;

        ResourceState       preState = isDrawingToMSRT ? RESOURCE_STATE_PIXEL_SHADER_RESOURCE : RESOURCE_STATE_SHADER_RESOURCE;
        RenderTargetBarrier barriers[] = { { pDepthRenderTarget, preState, RESOURCE_STATE_DEPTH_WRITE } };
        cmdResourceBarrier(cmd, 0, NULL, 0, NULL, 1, barriers);

        // Set load actions to clear the screen to black
        BindRenderTargetsDesc bindRenderTargets = {};
        bindRenderTargets.mRenderTargetCount = 1;
        bindRenderTargets.mRenderTargets[0] = { pDestinationRenderTarget, LOAD_ACTION_DONTCARE };
        bindRenderTargets.mDepthStencil = { pDepthRenderTarget, LOAD_ACTION_CLEAR, LOAD_ACTION_LOAD, STORE_ACTION_STORE,
                                            STORE_ACTION_NONE };
        cmdBindRenderTargets(cmd, &bindRenderTargets);
        ...
        Buffer* pIndexBuffer = pGeom->pIndexBuffer;
        cmdBindIndexBuffer(cmd, pIndexBuffer, INDEX_TYPE_UINT32, 0);

        Buffer*  pVertexBuffers[3] = { pGeom->pVertexBuffers[0], pGeom->pVertexBuffers[1], pGeom->pVertexBuffers[2] };
        uint32_t vertexStrides[3] = { 12, 4, 4 };
        uint64_t vertexOffsets[3] = { 0, 0, 0 };
        cmdBindVertexBuffer(cmd, 3, pVertexBuffers, vertexStrides, vertexOffsets);

        const char* profileName = "Depth Pre Pass";
        cmdBeginGpuTimestampQuery(cmd, pGpuProfiler, profileName);

        cmdBindPipeline(cmd, pPipelineDepthPrePass);
        cmdDrawIndexed(cmd, pGeom->mIndexCount, 0, 0);
        cmdEndGpuTimestampQuery(cmd, pGpuProfiler);

        cmdBindRenderTargets(cmd, NULL);

        RenderTargetBarrier barriers2[] = { { pDepthRenderTarget, RESOURCE_STATE_DEPTH_WRITE, RESOURCE_STATE_DEPTH_READ } };
        cmdResourceBarrier(cmd, 0, NULL, 0, NULL, 1, barriers2);
    }

    // Render the scene to show overdraw. In this pass the scene geometry is rendered
    // simply to demonstrate overdraw still visible after depth pre pass
    void drawSimpleGeometry(Cmd* cmd, ProfileToken pGpuProfiler)
    {
        const bool    isDrawingToMSRT = gAppSettings.mMsaaLevel > SAMPLE_COUNT_1;
        RenderTarget* pDestinationRenderTarget = isDrawingToMSRT ? pRenderTargetMSAA : pScreenRenderTarget;
        RenderTarget* pDepthRenderTarget = isDrawingToMSRT ? pMSAAEdgesStencilBuffer : pDepthBuffer;

        if (!gAppSettings.mUseDepthPrePass && isDrawingToMSRT)
        {
            RenderTargetBarrier barriers[] = { { pDepthRenderTarget, RESOURCE_STATE_PIXEL_SHADER_RESOURCE, RESOURCE_STATE_DEPTH_WRITE } };
            cmdResourceBarrier(cmd, 0, NULL, 0, NULL, 1, barriers);
        }
        else if (!gAppSettings.mUseDepthPrePass && !isDrawingToMSRT)
        {
            RenderTargetBarrier barriers[] = { { pDepthRenderTarget, RESOURCE_STATE_SHADER_RESOURCE, RESOURCE_STATE_DEPTH_WRITE } };
            cmdResourceBarrier(cmd, 0, NULL, 0, NULL, 1, barriers);
        }
        ...
        if (gAppSettings.mUseDepthPrePass)
        {
            bindRenderTargets.mDepthStencil = { pDepthRenderTarget, LOAD_ACTION_LOAD, LOAD_ACTION_LOAD, STORE_ACTION_STORE,
                                                STORE_ACTION_NONE };
        }
        else
        {
        ...
        const char* profileName = "Simple Geometry";
        cmdBeginGpuTimestampQuery(cmd, pGpuProfiler, profileName);
        ...
        {
            cmdBindPipeline(cmd, pPipelineSimpleGeometryDepthWrite);
        }
        cmdDrawIndexed(cmd, pGeom->mIndexCount, 0, 0);
        cmdEndGpuTimestampQuery(cmd, pGpuProfiler);
        ...
        // No barrier needed if MSAA and Depth Pre Pass is used
        if (!(isDrawingToMSRT && gAppSettings.mUseDepthPrePass))
        {
            ResourceState preState, postState;

            if (!gAppSettings.mUseDepthPrePass)
            {
                preState = RESOURCE_STATE_DEPTH_WRITE;
                postState = isDrawingToMSRT ? RESOURCE_STATE_DEPTH_READ : RESOURCE_STATE_SHADER_RESOURCE;
            }
            else // Depth Pre Pass, but no MSAA
            {
                preState = RESOURCE_STATE_DEPTH_READ;
                postState = RESOURCE_STATE_SHADER_RESOURCE;
            }

            RenderTargetBarrier barriers2[] = { { pDepthRenderTarget, preState, postState } };
            cmdResourceBarrier(cmd, 0, NULL, 0, NULL, 1, barriers2);
        }
    }
    ...
    void drawScene(Cmd* cmd, uint32_t frameIdx)
    {
        ...
        if (gAppSettings.mUseDepthPrePass)
        {
            cmdBeginGpuTimestampQuery(cmd, gGraphicsProfileToken, "Depth Pre Pass");

            depthPrePass(cmd, gGraphicsProfileToken);

            cmdEndGpuTimestampQuery(cmd, gGraphicsProfileToken);
        }

        cmdBeginGpuTimestampQuery(cmd, gGraphicsProfileToken, "Simple Geometry Forward Rendering Pass");

        drawSimpleGeometry(cmd, gGraphicsProfileToken);
